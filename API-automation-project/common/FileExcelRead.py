...
class FileExcelRead():
    ...
    def read_excel(excel_file_path=EXCEL_FILE_PATH6, sheet_name=SHEET_NAME):
        ...
        # 获取到excel文件后，判断传入的sheet_name是不是在excel文件中,如果存在，就加载该sheet页的数据，work.sheetnames是获取excel的所有sheet页
        ...
        # todo 获取列名，把sheet页第2行的数据拿到当做key值，也就是列名，返回数据格式是列表，列表中的元素是每一列的列名
        ...
        # todo 遍历excel中的从第三行开始的每一行数据，将excel的每一行数据当做value,存储为一个字典，并且放在data中
        data = []  # data是需要执行的用例数据
        ...
        workbook.close()
        # data的数据格式是列表嵌套字典，每个字典对应一行数据（也就是测试用例），字典的key是列名，value是每一行对应的数据
        return data
